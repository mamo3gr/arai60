...
まず、隣接する単語の探索については、メモリと速度とのトレードオフにグラデーションがある。

- **毎回計算する：** （step1,2での解法）
  - 単語vs.単語で隣接している・していないの判定に `O(L)` の時間がかかる。各層で、単語ひとつにつき他の単語と突き合わせていると `O(LW)` かかる。
  - 特に保持しておく情報はない
    - キャッシュを使うと速いかも
- **サブワード、あるいはワイルドカードをkeyとして辞書を作る：**
  - はじめに `O(LW)` の時間がかかる。
  - 単語あたり `O(L)` で隣接する単語を引ける。
  - 空間計算量について、単語あたり `L` 個のkey, valueひとつができる。つまり `O(LW)`
- **隣接グラフを作る：**
  - はじめに `O(LW^2)` の時間がかかる。
  - その後は、ある単語に対し、隣接する単語を `O(1)` で引ける。
  - 空間計算量は `O(W^2)` 必要。

明確なしきい値があるわけではないが、隣接グラフを作るのはオーバーすぎるように思う。辞書を作る方式がバランスが取れているように感じる。

次に、BFSを双方向でやるか、片方向でやるか、がある。高いパフォーマンスが求められるなら双方向をやる必要があるが、実装が比較的シンプルになることからまずは片方向を選択すると思う。  
片方向の場合は、(1) キューに `(単語, 距離=深さ)` のタプルを入れていく方法と、(2) 深さごとにループを区切る、がある。(2) のほうがdequeが不要だし、いまどの深さか？が重複なく保持できるので好みに思う。
