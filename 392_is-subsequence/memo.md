...
`for c in s` について `t` から `c` を探す、というのが自然に感じるが、
`t_i` をどこかでインクリメントする、というのがややトリッキーに思える（いつかミスしそう）。
コード的にはダブルポインタの方が間違いが少なさそう。

## フォローアップ問題の再訪

https://github.com/mamo3gr/arai60/pull/52#discussion_r2864553208

>Gemini の解法に違和感があったため、自分でも解いてみました。
>
>index_and_char_to_index[t_index][ch] = t_index 以降で直近で ch が現れるインデックス + 1 という  
>テーブルを作ってあげると、 s の中の 1 文字あたりの処理の時間計算量が O(1) となり、  
>s 一つあたり O(|s|) となると思います。

C++の実装を参考に、Pythonでも書いてみる (step4.py). 主な変更点は以下。

* `ch` を関数 `char_to_index` で 0-25 にマッピングする
* 「インデックス + 1」ではなく、「インデックス」にする
  * `index_and_char_to_index` の説明として、前者では読解がより難しいと感じたため
  * 代わりに、探索中で見つかるたび `t_index` をインクリメントする
