...
ただしデメリットとして、最後にインデックスを調べるときには、-1でない値が出てくるまでdictをループして取り出す必要がある。重複する文字が多いときは比較的時間がかかる。文字列は最長で 10^5. 多少複雑になっても、最悪ケースの時間を考えると最初の方針のほうが優れていそう。
...
step1では、「1回以上登場した文字」と「文字ごとの初出の位置」を格納していたが、前者を「 **2回** 以上登場した文字」にするとシンプルになる。  
最後に結果を取り出すにはリスト化を防ぐために `next(iter(...` のようにする必要があるが、読みづらい。
